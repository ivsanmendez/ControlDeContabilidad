...
- House report page (#7, PR #12, merged 2026-06-14) — report page at `/houses/:id/report` showing all house data: assigned users, access controls, vehicles, contributors with monthly payment history, and a yearly income summary. Static QR code per house (no year parameter). Vehicle hang tag generator: a position picker dialog (3x3 grid, slots 1-9) opens an A4 print page with only the selected slot filled, allowing reuse of partially-used sticker sheets; reference image at `docs/hangtag-slot-order-A4.jpeg`
