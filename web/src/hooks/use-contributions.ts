...
export function useContributions(contributorId?: number, year?: number, houseId?: number) {
  const params = new URLSearchParams()
  if (houseId) {
    params.set('house_id', String(houseId))
  } else {
  ...
  return useQuery<ContributionDetail[]>({
    queryKey: ['contributions', contributorId ?? '', year ?? '', houseId ?? ''],
